...
以下為參考專案——每一個都源自實際遇到的問題，記錄了「我如何思考、工作」，而不只是「做了什麼」：包含決策過程、權衡取捨、工具選型，以及維運面的考量。
...
Reference projects — each one came out of a real problem and documents *how I reason and work*, not just what was built: the decisions, the trade-offs, the tooling choices, and the operational considerations.
